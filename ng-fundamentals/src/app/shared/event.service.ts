...
export class EventService {
  ...
  getEvents(): Observable<Event[]> {
    // we can transform our data after map()
    // this is useful to make sure shape is correct
    // when we retrieve the data on the server.
  ...
  getEvent(id: number): Observable<Event> {
    // we can use map(map((response) => <Event>response))
    // to transform data type, but we can also use this
    // where we specify the return value of the get() method
    return this.http.get<Event>(`/api/events/${id}`).pipe(
      catchError(this.handleError('getEvents', <Event>{}))
    )
  }
  ...
  saveEvent(event: Event) : Observable<Event> {
    ...
    .pipe(catchError(this.handleError('saveEvent', <Event>{})))
  }
